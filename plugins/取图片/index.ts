  ...
  setup: (ctx) => {
    ctx.handle("message", async (e) => {
      if(e.raw_message.match("取图片")){
        // 从当前消息取图片
        let imageUrl = e.message.filter((value) => value.type === "image")[0]?.data.url
        // 从引用消息取图片
        if(!imageUrl){
          const quoteMessageId = parseInt(e.message.filter(value => value.type === 'reply')[0]?.data.id)
          if(e.message_type === "group"){
            const historyMessage = await ctx.bot.get_group_msg_history({group_id: e.group_id})
            const message = historyMessage.messages.filter(value => value.message_id === quoteMessageId)[0]
            imageUrl = message.message.filter(value => value.type === 'image')[0]?.data.url
          } else {
